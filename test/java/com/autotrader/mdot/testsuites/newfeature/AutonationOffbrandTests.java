 ...
 */
public class AutonationOffbrandTests {
    ...
    String AutoNationLinkText = "Visit AutoNation";
    String AutoNationTileImageUrl = "/certified/offbrand-tiles/245941.png";
    String AutoNationUrl = "http://recallfreecars.com/";
    DesiredCapabilities capability;
    String copy =
        "Why Choose us?\n\n"
            + "Only AutoNation Cars are Guaranteed to be Recall Free\n\n"
            + "Every car AutoNation sells is checked to make sure that every item that any manufacturer identified in a recall has been repaired, replaced or corrected. So every car we sell, new or used, is guaranteed to be Recall Free.\n\n"
            + "We care about customer's safety and their peace-of-mind. That's why we make buying at AutoNation easy and worry free.";
    ...
    /**
     * Should verify link redirect B-17231 - Navigate User to AutoNation offsite on click
     *
     * @throws InterruptedException on error
     */
    @Test(priority = 4, groups = "AutoNationTests", dependsOnMethods = "shouldVerifySearchResults")
    public void shouldNavigateUserOffsite() throws InterruptedException {
        ...
        Assert.assertEquals(driver.getCurrentUrl(), AutoNationUrl);
    }

    /**
     * Should verify AutoNation link B-17372 - Display "Visit AutoNation" text link on Warranty Accordion on mobile VDP
     */
    @Test(priority = 2, groups = "AutoNationTests", dependsOnMethods = "shouldVerifySearchResults")
    public void shouldVerifyAutoNationlink() {
        driver.get(listingUrl);
        vehicleDetailsPage.expandWarrantySection();
        waitFor.waitForExpectedConditions(driver, By.cssSelector(".visit.ui-link"));
        Assert.assertTrue(vehicleDetailsPage.getWarrantyTabLinkText().equals(AutoNationLinkText));
    }

    /**
     * Should verify AutoNation tile on Car Info B-17368 - Display AutoNation tile under car info on mobile VDP
     */
    @Test(priority = 1, groups = "AutoNationTests", dependsOnMethods = "shouldVerifySearchResults")
    public void shouldVerifyAutoNationTileOnCarInfo() {
        driver.get(listingUrl);
        waitFor.waitForExpectedConditions(driver, By.cssSelector(".atc-image.cpoTile"));
        Assert.assertTrue(vehicleDetailsPage.getURLForCPOTileOnCarInfo().contains(AutoNationTileImageUrl));
    }

    /**
     * Should verify the AutoNation CPO tile on warranty accordion B-17370 - Display AutoNation tile on Warranty
     * Accordion on mobile VDP
     */
    @Test(priority = 2, groups = "AutoNationTests", dependsOnMethods = "shouldVerifySearchResults")
    public void shouldVerifyAutoNationTileOnWarrantyAccordion() {
        driver.get(listingUrl);
        vehicleDetailsPage.expandWarrantySection();
        Assert.assertTrue(vehicleDetailsPage.getURLForCPOTileOnWarrantyAccordion().contains(AutoNationTileImageUrl));
    }
    ...
    public void shouldVerifySearchResults() throws InterruptedException {
        ...
        locationAndRadiusSliderPage.setZip("95136");
        ...
        moreOptionsSliderPage.enterKeywords("1ZVBP8JZ3E5258679");
    ...
    /**
     * Should verify copy on warranty tab B-17371 - Display AutoNation Warranty copy on Warranty Accordion on mobile VDP
     */
    @Test(priority = 2, groups = "AutoNationTests", dependsOnMethods = "shouldVerifySearchResults")
